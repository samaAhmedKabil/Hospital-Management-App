...
class AdapterReports(): RecyclerView.Adapter<AdapterReports.Holder>() {

    var data:List<ReportsData> ? = null
    ...
    fun updateList(newItems: List<ReportsData>) {
        data = newItems
    ...
    inner class Holder(private val binding: ItemCallsBinding) : RecyclerView.ViewHolder(binding.root)
    {
        ...
        fun bind(reportsData: ReportsData){
            binding.name.text = reportsData.report_name
